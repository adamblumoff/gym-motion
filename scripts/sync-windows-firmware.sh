...
SOURCE_DIR="$(dirname "$SOURCE_SKETCH")"
WINDOWS_DIR="$(dirname "$WINDOWS_SKETCH")"
...
mkdir -p "$WINDOWS_DIR"
find "$WINDOWS_DIR" -maxdepth 1 -type f -name '*.ino' -delete

cp "$SOURCE_SKETCH" "$WINDOWS_SKETCH"

while IFS= read -r -d '' source_file; do
  if [[ "$source_file" == "$SOURCE_SKETCH" ]]; then
    continue
  fi
  target_file="$WINDOWS_DIR/$(basename "$source_file")"
  cp "$source_file" "$target_file"
done < <(find "$SOURCE_DIR" -maxdepth 1 -type f -name '*.ino' -print0)
...
if cmp -s "$SOURCE_SKETCH" "$WINDOWS_SKETCH"; then
  ...
  echo "  tabs: $(find "$SOURCE_DIR" -maxdepth 1 -type f -name '*.ino' | wc -l)"
else
