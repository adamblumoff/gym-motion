...
- The sidecar should also give the early WinRT GATT handshake a few bounded retries. If `connect()` or `discover_services()` flakes during reconnect, the sidecar should retry that low-level setup before burning a whole reconnect attempt.
