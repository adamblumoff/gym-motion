...
- Separate guardrail: even after scan pause is correct, WinRT can still surface a rediscovered device before its first uncached GATT fetch is stable. On this stack, `btleplug` implements `connect()` by issuing an uncached `GetGattServices...` request, so a paired node may briefly look rediscovered while `connect()` still returns `Not connected`. Treat that as a short transport warm-up window: wait briefly after scan stop, refresh the peripheral handle again, and only then start the reconnect GATT retries.
