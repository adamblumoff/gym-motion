...
- The firmware keeps a short pre-bootstrap watchdog after a client subscribes to the runtime telemetry characteristic, not after every BLE connection. That still lets the Windows app recover from a crash before `app-session-bootstrap`, but provisioning-only sessions should stay connected and never be treated like runtime sessions just because they share the same BLE server.
