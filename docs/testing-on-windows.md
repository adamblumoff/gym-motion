...
- Managed firmware now tracks a Windows app-session lease for runtime sessions. The sidecar refreshes that lease every 5 seconds once runtime control traffic begins, and the node expires it after 15 seconds so it can drop a stale BLE session, restart advertising, and recover from app restarts that do not produce a clean OS-level disconnect. Provisioning-only BLE sessions stay outside that lease watchdog.
