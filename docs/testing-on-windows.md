...
- Managed firmware now tracks a Windows app-session lease for Windows runtime sessions. The sidecar refreshes that lease every 5 seconds once `app-session-lease` traffic begins, and the node expires it after 15 seconds so it can drop a stale BLE session, restart advertising, and recover from app restarts that do not produce a clean OS-level disconnect. Provisioning-only sessions and legacy non-Windows runtime commands stay outside that lease watchdog.
