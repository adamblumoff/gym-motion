...
- Firmware serial logs now spell out the runtime transport lifecycle. During bench testing, look for `[runtime] BLE client connected; waiting for Windows app session lease.`, `[runtime] Windows app session lease is active...`, `[runtime] Lease refreshed. ...`, `[runtime] Connected heartbeat. connected=1 leased=1 ...`, `[runtime] Lease expiry timeout fired. ...`, `[runtime] BLE runtime transport disconnected from the Windows app.`, and `[runtime] Still waiting for the Windows app; BLE advertising is active.` to distinguish a real app session from a stale or missing client.
