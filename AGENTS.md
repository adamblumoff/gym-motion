...
- Windows desktop BLE now uses the native Rust WinRT sidecar so the built-in Windows Bluetooth adapter works. The older noble + WinUSB desktop runtime path has been removed from this repo; do not reintroduce it as a fallback.
...
- Windows BLE bug-memory note: read `/home/adamblumoff/gym-motion/docs/bugs/windows-ble-bugs.md` before changing reconnect flow, app-session lease behavior, or the vendored WinRT `btleplug` package. That file is the source of truth for known reconnect/session bugs, root causes, and recovery rules.
...
- The desktop BLE runtime in this repo is Windows-only. Bench or non-Windows experiments should live outside the product runtime path and should not drive product decisions.
...
- Approved-node identity policy note: keep one shared source of truth for matching order (`knownDeviceId`, then `peripheralId`, then BLE address, then unique `localName`). Do not re-implement that policy separately in setup UI, setup merging, or other Electron helpers.
