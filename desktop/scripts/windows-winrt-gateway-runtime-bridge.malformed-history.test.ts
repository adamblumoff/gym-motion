...
describe("windows winrt gateway runtime bridge malformed history recovery", () => {
  ...
  it("drops duplicate history sequences within a page and persists only the unique records", async () => {
    const sidecarCommands = [];
    const persistedBodies = [];
    ...
    expect(persistedBodies[0].records).toHaveLength(3);
    expect(persistedBodies[0].records.map((record) => record.sequence)).toEqual([
      783,
      1451,
      1586,
    ]);
    expect(persistedBodies[0].records[0].kind).toBe("node-log");
